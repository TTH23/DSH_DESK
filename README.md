...
- 💰 **用量监控**：托盘菜单显示 DeepSeek 账户**余额**（官方 `user/balance` 接口，账户级）
  ...
  - 计价口径：官方峰谷定价（元/百万 tokens）——
    **deepseek-flash**：缓存命中 0.02/0.04、缓存未命中 1/2、输出 4/8（空闲/高峰）；
    **deepseek-v4-pro**：命中 0.15/0.30、未命中 4.5/9、输出 13.5/27（空闲/高峰）。
    **高峰时段 = 北京时间周一至周五（不含中国法定节假日）9:00–12:00、14:00–18:00**；
    其余时段（含**周末与法定节假日全天**）为空闲，空闲价 = 高峰价的一半。
    托盘状态行与悬停提示用 ☼（高峰）/ ☾（空闲）标注当前时段。
    模型名以官方最新为准（`deepseek-v4-flash` 等旧名仍可调用，按 flash 计费）。
